...
void night() {
    ...
    glClear(GL_COLOR_BUFFER_BIT);
    Origin();
    glClearColor(0, 0, 0.075, 0.0);//sky
...
void second() 
{
    ...
    if (doorOpen) {
        glTranslatef(boyPosX+0.05 + boyPosX2, boyPosY + boyPosY2, 0);
        glScalef(.08 + schoolBoyScalVal, .08 + schoolBoyScalVal, 1);
        schoolBoy();
        Origin();
    }
...
void welcomeDisplay() 
{
    ...
    glRasterPos2d(-0.5, 0.75);
    char s[] = "Computer Graphics and Visulatization Mini project";
    for(int i=0;i<strlen(s);i++)
        glutBitmapCharacter(GLUT_BITMAP_9_BY_15, s[i]);

    glRasterPos2d(-0.5, 0.5);
    char s1[] = "Topic:City simulation";
    for (int i = 0;i < strlen(s1);i++)
        glutBitmapCharacter(GLUT_BITMAP_8_BY_13, s1[i]);

    glRasterPos2d(-0.5, 0.4);
    char s2[] = "Pratham P Kotian(4SF19CS115) & Pujari Sanmith Anand(4SF17CS121)";
    for (int i = 0;i < strlen(s2);i++)
        glutBitmapCharacter(GLUT_BITMAP_8_BY_13, s2[i]);
    
    glRasterPos2d(-0.5, 0.3);
    char s3[] = "Under the guidance of Mrs Vidya VV, Assistant Professor. Dept of CS&E";
    for (int i = 0;i < strlen(s3);i++)
        glutBitmapCharacter(GLUT_BITMAP_8_BY_13, s3[i]);

    glRasterPos2d(-0.5, 0.2);
    char s4[] = "Sahyadri College of Engineering and Management";
    for (int i = 0;i < strlen(s4);i++)
        glutBitmapCharacter(GLUT_BITMAP_8_BY_13, s4[i]);

    glRasterPos2d(-0.5, -0.5);
    char s5[] = "PRESS s to start";
    for (int i = 0;i < strlen(s5);i++)
        glutBitmapCharacter(GLUT_BITMAP_8_BY_13, s5[i]);
...
int main(int argc, char* argv[])
{
    ...
    glutCreateWindow("City simulation"); // Create a window with the given title
